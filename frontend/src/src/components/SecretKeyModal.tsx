...
const SecretKeyModal = ({ publicKey, secretKey, onSaved }: SecretKeyModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-surface border border-border rounded-2xl max-w-lg w-full p-8 shadow-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center bg-slate-900 border border-border rounded-lg overflow-hidden">
              <code className="flex-1 px-4 py-3 text-sm font-mono text-text-primary">{publicKey}</code>
              <button
                onClick={() => copyToClipboard(publicKey)}
                className="px-4 py-3 border-l border-border hover:bg-white/5 transition-colors"
              >
                {copiedKey === publicKey ? <Check size={16} className="text-green-400" /> : <Copy size={16} className="text-text-muted" />}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center bg-slate-900 border border-border rounded-lg overflow-hidden">
              ...
              <button
                ...
                className="px-4 py-3 border-l border-border hover:bg-white/5 transition-colors"
              >
                {copiedKey === secretKey ? <Check size={16} className="text-green-400" /> : <Copy size={16} className="text-text-muted" />}
              </button>
            </div>
          </div>
        </div>
